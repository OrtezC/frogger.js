...
bgMusic.src = "backgroundMusic.ogg";
...
var player = {
  ...
  drawPlayer: function() {
    addImageComponent((canvas.width/16)*this.posX, (canvas.height/16)*this.posY, canvas.width/16, canvas.height/16, this.image);
  },
...
function endGame() {
  clearGame();
  gameOver.play();
  addImageComponent(0, 0, canvas.width, canvas.height, "background.svg");
  drawScore();
...
function resizeCanvas() {
  ...
  } else {canvas.height = 960}

  if (window.innerWidth/2 < 960) {
    canvas.width = window.innerWidth/2;
  } else {canvas.width = 960}
}
...
function drawEnemies(enemies) {
  enemies.forEach(function(element) {
    addImageComponent((canvas.width/16)*element.posX, (canvas.height/16)*element.posY, canvas.width/16, canvas.height/16, element.image);
  }); 
}
...
function playerControl(key) {
  switch(key.keyCode) {
    case 37: if(player.posX === 0) break; //left
      else {
        player.image = "frogLeft.png"
        player.posX--;
        playerWalk.play();       
        break;
      }

    case 38: if(player.posY === 0) break; //up
      else {
        player.image = "frogUp.png"
        player.posY--;
        playerWalk.play()         
        break;
      }
    case 39: if(player.posX === 15) break; //right
      else {
        player.image = "frogRight.png"
        player.posX++;
        playerWalk.play()          
        break;
      }

    case 40: if(player.posY === 15) break; //down
      else {
        player.image = "frogDown.png"
        player.posY++;
        playerWalk.play()          
        break;
      }
...
function drawScore() {
  ...
  var fontSize = (canvas.height/16)-8;
  ctx.font = fontSize + "px Verdana";
  ctx.fillText("Score: " + player.score,(canvas.width/16)*.2,(canvas.height/16)*.8);
  ctx.fillText("Lives: " + player.lives,(canvas.width/16)*8, (canvas.height/16)*.8);
}